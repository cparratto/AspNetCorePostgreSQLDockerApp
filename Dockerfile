...
RUN echo "deb [arch=amd64] https://apt-mo.trafficmanager.net/repos/dotnet/ trusty main" > /etc/apt/sources.list.d/dotnetdev.list \
    ...
    && apt-get update \
    && DEBIAN_FRONTEND=noninteractive apt-get -q -y install libclang1-3.5 clang-3.5libc6 libcurl3 libgcc1 libssl1.0.0 libstdc++6 libtinfo5 libuuid1 zlib1g \
    && DEBIAN_FRONTEND=noninteractive apt-get -q -y installdotnet-sharedframework-microsoft.netcore.app-$DOTNET_CORE_VERSION dotnet-dev-$DOTNET_CORE_SDK_VERSION
RUN mkdir -p /app
COPY . /app/
